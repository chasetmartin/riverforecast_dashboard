<script lang="ts">
    ...
    const chartdata = {
        labels: observedLabels.concat(forecastLabels),
        datasets: [
            {
                name: "Cubic Feet per Second (CFS)",
                chartType: "line",
                values: observedValues.concat(forecastValues)
            },
            // {
            //     name: "Forecast (CFS)",
            //     chartType: "line",
            //     values: forecastData.map((entry: { secondary: number; }) => entry.secondary * 1000)
            // }
        ],
        yMarkers: [
                {
